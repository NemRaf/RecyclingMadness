...
public class Sea extends World
{
    ...
    public void act(){
        ...
        for(Plastic obj: getObjects(Plastic.class))
        {
    ...
    private void createPlastic(){
        CoffeCup coffee = new CoffeCup();
        FaceMask mask = new FaceMask();
        PlasticBottle bottle = new PlasticBottle();
        int random= getRandomNumber(50,450);
        int random1 = getRandomNumber(0,3);
        if (random1 ==0)
        {
            GreenfootImage image = coffee.getImage();
            addObject(coffee,getWidth(),random);
        }else if(random1==1)
        {
            GreenfootImage image = coffee.getImage();
            addObject(mask,getWidth(),random);
        }else
        {
            GreenfootImage image = coffee.getImage();
            addObject(bottle,getWidth(),random);
        }
